...
sub reDim
{
  ...
  my @dims = qw (D%NIT D%NJT);

  for my $dim (@dims)
    {
      my @en_decl = &F ('.//EN-decl[./array-spec/shape-spec-LT[string(shape-spec)="?"]]', $dim, $d);
          ...
          for my $ss (@ss[1..$#ss])
            {
              ...
              for my $bb ($lb, $ub)
                {
                  next unless ($bb);
                  next EN_DECL unless ($bb->nodeName eq 'literal-E' || grep { $bb->textContent eq $_ } @dims);
                }
            }
