...
& PHLI_HRI_OUT, PHLI_HCF_OUT, LLVERBOSE, CLCASE)
...
IF (LLSTAT) THEN
  WRITE (*, '(A32," ",6E15.6)') TRIM (CDNAME), &
& MINVAL (PREF), MAXVAL (PREF), SUM (PREF) / SIZE (PREF), &
& MINVAL (POUT), MAXVAL (POUT), SUM (POUT) / SIZE (POUT)
ELSE
  PRINT *, CDNAME
ENDIF
